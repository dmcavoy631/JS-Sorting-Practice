...
let indexTest = stringsToNumbs(strNums)
// Find the index of the first value when added to it's index = 512 (#ATX!!)
let atxIdx = 0;

for (let i = 0; i < indexTest.length; i++){
  if (i + indexTest[i] === 512){ //  Is the expected result 0, these are single digit values?  
    atxIdx = i;
  }
}


console.log(`index: ${atxIdx}, value: ${indexTest[atxIdx]}`);
...
for(let ele of weather){
     weatherStates.push(ele.weather_state_name)
  }
...
const idealTemp = [];
for (let ele of weather){
  if (ele.min_temp === 15.915) idealTemp.push(ele.id);
  }
